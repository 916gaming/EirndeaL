...
const socket = io('https://monopoly-deal-server.onrender.com');
...
export default function MonopolyDeal() {
  ...
  const playCard = (cardIndex) => socket.emit('playCard', { roomId, cardIndex });
    ...
    <div className="p-4 max-w-xl mx-auto text-center">
      ...
      {!inRoom ? (
        <div className="space-y-2">
          ...
          <input className="border p-2 w-full" placeholder="Room ID" value={roomId} onChange={(e) => setRoomId(e.target.value)} />
          <div className="flex gap-2 justify-center">
            <button className="bg-green-500 text-white px-4 py-2" onClick={createRoom}>Create Room</button>
            <button className="bg-blue-500 text-white px-4 py-2" onClick={joinRoom}>Join Room</button>
          </div>
        </div>
      ) : !gameStarted ? (
        <div>
          ...
          <button className="bg-purple-600 text-white px-6 py-2" onClick={startGame}>Start Game</button>
        </div>
      ) : (
        <div>
